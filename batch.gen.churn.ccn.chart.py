...
def get_churn_complexity(df, data_type, prefix):
	churn_complexity = []
	repo = ''
...
def generate_all_repos_chart(git_repo_list, csv_file_list, data_type):
	common_prefix = get_common_prefix(git_repo_list)
	for index, csv_filepath in enumerate(csv_file_list):
		df        = read_csv_file(csv_filepath)
		repo_name = get_last_dirname(git_repo_list[index])
...
def write_all_files(cumulative_df, data_type, result_csv_filename):
	complexity = data_type + ' complexity'
	df = cumulative_df.sort(
		['commits', 'file complexity', complexity, 'filename'], ascending=[False, False, False, True])
...
generate_all_repos_chart(git_repo_list, csv_file_list, data_type)
print " ... Done"
